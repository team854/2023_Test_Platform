...
/**
 * Game Controller extends {@link XboxController}
 * <p>
...
public class GameController extends XboxController {
     ...
     */
    public GameController(int port, double axisDeadband) {
        super(port);
    ...
    public void setAxisDeadband(double axisDeadband) {
        ...
        if (axisDeadband < 0 || axisDeadband > 0.4) {
            System.out.println("Invalid axis deadband(" + axisDeadband
                + ") must be between 0 - 0.4. Axis deadband value not changed.  "
                + "Currently " + this.axisDeadband);
            return;
        }
